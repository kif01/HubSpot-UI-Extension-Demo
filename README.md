...
**4.3.** Grab the code from this [crm-card.js](https://github.com/kif01/HubSpot-UI-Extension-Demo/blob/main/src/app/app.functions/crm-card.js) in this repo and paste it in your **crm-card.js**. Basically this code get the deals of a specific contact calculate the amount of closed deals, gets the number of closed deal and number of ongoing deals and displays them on the card. In addition it has a label that changes  dynamically based on the closed deals amount. For example if this amount is above a specific threshold then it shows that this is a **High Value Customer** label and displays the VIP image.  </br></br>
